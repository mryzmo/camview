...
from imgviewplif import ImageViewPLIF, MplCmapImageViewPLIF
import pickle
    ...
    def initUI(self):
        ...
        params = [
            ...
            {'name': 'PLIFFile', 'type': 'group', 'children': [
                ...
                {'name': 'BeginFrame', 'type': 'int', 'value': 170},
                {'name': 'EndFrame', 'type': 'int', 'value': 250},
...
# viewbox setLimits
        ...
        def save():
            ...
            with open(self.p.child('PLIFFile', 'RealFileName').value()+'.val', 'wb') as f:
                pickle.dump(state, f)

        def restore():
            global state
            add = self.p['Save/Restore functionality',
                         'Restore', 'Add missing items']
            rem = self.p['Save/Restore functionality',
                         'Restore', 'Remove extra items']
            with open(self.p.child('PLIFFile', 'RealFileName').value()+'.val', 'rb') as f:
                state = pickle.load(f)
        ...
        def redrawSheet():
            ...
            img.setImage(invfactor*self.showData[:, :, top:bottom],
                         autoHistogramRange=self.p.child(
                             'Geometry', 'Autoscale').value(),
